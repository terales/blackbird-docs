---
  locale: uk
  title: Crowdin
  description: Додаток Blackbird для Crowdin
---
import { LinkCard } from "@astrojs/starlight/components";

<LinkCard title="View on Github" target="_blank" href="https://github.com/bb-io/Crowdin" icon="github" />

Crowdin — це хмарна платформа для локалізації, яка допомагає бізнесам та організаціям керувати перекладом їхнього програмного забезпечення, додатків, веб-сайтів та іншого цифрового контенту на багато мов. Вона надає середовище для співпраці, де перекладачі та менеджери проєктів можуть працювати разом для ефективного перекладу та локалізації контенту.

## Перед підключенням

1. Переконайтеся, що у вас є доступ до облікового запису на екземплярі Crowdin, до якого ви хочете підключитися.
2. Якщо ви хочете використовувати події, переконайтеся, що ваш обліковий запис має достатні права для створення вебхуків.

## Підключення

1. Перейдіть до додатків і знайдіть Crowdin. Якщо ви не можете знайти Crowdin, натисніть _Add App_ у верхньому правому куті, виберіть Crowdin і додайте додаток до вашого середовища Blackbird.
2. Натисніть _Add Connection_.
3. Назвіть ваше підключення для подальшого використання, наприклад, 'Моє підключення до Crowdin'.
4. Виберіть ваш план Crowdin. Це може бути Basic або Enterprise. Дізнайтеся більше на https://crowdin.com/pricing
5. Натисніть _Authorize connection_.
6. Коли відкриється нове вікно, дотримуйтеся інструкцій від Crowdin.
7. Коли ви повернетеся до Blackbird, переконайтеся, що підключення з'явилося і його статус — _Connected_.
...
> Зверніть увагу, що додаток працює з обома планами Crowdin: Enterprise та Basic. Однак деякі дії недоступні в плані Basic. Ці дії позначені як [Enterprise] на початку їхньої назви і доступні лише для плану Enterprise.
...
Хочете використовувати цей додаток або маєте відгуки щодо нашої реалізації? Зв'яжіться з нами через [встановлені канали](https://www.blackbird.io/) або створіть питання.